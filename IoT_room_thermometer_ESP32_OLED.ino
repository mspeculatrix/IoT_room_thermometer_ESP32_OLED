...
void printIP() {
  ...
  uint8_t numChars = ip.toString().length() + 1;
  char buf[numChars];
...
void printLine(String text, uint8_t line) {
  /* Prints text using the OLED displays default font, size 1 */
  uint8_t ypos = (line - 1) * LINE_HEIGHT_1 + 1;
  display.setFont();        // don't use GFX fonts
  display.setTextSize(1);
  display.fillRect(1, ypos, SCREEN_WIDTH, LINE_HEIGHT_1, BLACK);  // erase previous reading
  display.setCursor(1, ypos);
  display.print(text);
  display.display();
}

void printTemperature(float newtemp) {
...
void printHumidity(float hum) {
  ...
  display.setFont();          // stop using GFX fonts
  display.setTextSize(1);
  sprintf(buf, "hum: %.0f%%", hum);
  int16_t  x1, y1; uint16_t w, h;
  display.getTextBounds(buf, 1, 1, &x1, &y1, &w, &h);
  uint8_t ypos = SCREEN_HEIGHT-(h*2)-5;
  display.fillRect(1, ypos, SCREEN_WIDTH, LINE_HEIGHT_1, BLACK);  // erase previous reading
...
void loop() {
  ...
  if(report_counter == REPORT_INTERVAL) {
    ...
    if(wifiConnected) {
      ...
      Serial.println("Could not send report because wifi is down.");
    }
    report_counter = 0;
  } else {
    report_counter++;
